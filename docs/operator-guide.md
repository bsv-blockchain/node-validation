...
- Go 1.22 or later (only if rebuilding fixtures; `go.mod` requires 1.23)
...
> **Version-drift caveat.** The compose stack pins `ghcr.io/bsv-blockchain/teranode:v0.15.0-beta-2`. SP2 discovery (the source-of-truth for endpoint shapes the tests rely on) was performed against commit `11f5fa6a81c36490e2796561f76a39294fc422b5` from a feature branch. The pinned image may have been built from a different commit; this is documented in the README's "Version note" section. If specific tests fail with "endpoint not found" or "metric absent", that's the most likely cause — see the troubleshooting section.
